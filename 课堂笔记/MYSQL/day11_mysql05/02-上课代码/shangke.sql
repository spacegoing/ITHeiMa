...
select name,sex into outfile "C:/Users/Ricky/Desktop/b.txt"
from php_student;

load data infile "C:/Users/Ricky/Desktop/b.txt"
into table php_student(name,sex);

select * from php_student;
select * from php_class;

select * from php_student
where class_id in (select id from php_class);

#结果也是要查找 已经开班的人员信息
#exists
# exists 的子句是 查找 php_student.class_id = php_class.id 相对
#exists可以理解成为一个开关
select * from php_student 
where exists 
(select * from php_class where php_student.class_id = php_class.id);

select * from php_student 
where exists 
(select * from php_class where php_student.id = php_class.id);

select * from php_student 
where exists ( select * from php_student); -- where 1;


#创建一个视图
#表名 如果公司没有特殊规定,最好以 t_ 开头 
#建议视图名字以 vm_ 开头
create view vm_first as 
select * from php_student;

#视图就是一个虚拟表,那也是一个表,那查询的时候和对表查询一样
select * from vm_first;

select max(age) from vm_first;

#查看视图创建语句
show create view vm_first;


#修改视图,只能修改 后边的子查询
#不能修改视图的任何东西
alter view vm_first as 
select name from php_student;
...
#创建一个视图
create view vm_all_learn_student as 
select * from php_student 
where class_id in 
(select id from php_class);

#在虚拟表的基础上再进行操作
select * from vm_all_learn_student 
order by score desc;


#视图可以指提供部分表
create view vm_all_learn_student_part as 
select name,age,score from php_student;

# select语句中可以给字段 通过as其别名
create view vm_all_learn_student_other_name as 
	select name as othername from php_student;


create view vm_student_score as 
	select score+10 as score  from php_student;

# 水平分表 -- 表结构一致 一般union使用水平分表
create view vm_shuiping as 
(select * from php_student order by score desc limit 1)
union 
(select * from php_student order by score asc limit 1);
...
select * from php_student order by age group by class_id;
-- 这句话是错误的,因为 order by 必须在 group by 后边

select * from php_student group by class_id order by age;
-- 这句 能不能得到正确的结果
-- 应该出现悟空 和蜘蛛精
-- 这条语句 虽然语法正确,但是不能出现正确的结果

select * from  (select * from php_student order by age desc) as source 
group by class_id;
-- 这句话能够出现正确的结果
...
#一个merge视图
#视图的算法是merge
create algorithm = merge view vm_merge as 
	select * from php_student order by age desc;


#一个temptable视图
#视图的算法是 temptable
create algorithm = temptable view vm_temp as 
	select * from php_student order by age desc;


#查询结果
# merge 会先把 sql语句合并,合并的是 创建视图时候的sql语句以及要查询的sql语句
#查询 merge算法的视图
select * from vm_merge group by class_id;
# select * from php_student  group by class_id order by age desc;


#查询 temptable算法的视图
#temptable 会先执行 视图查询出一个表,这个表是一个临时表,
# 可以我们理解成为 作为一个数据源
# 然后再执行现在视图中的sql语句

select * from vm_temp group by class_id;
# select * from (select * from php_student order by age desc) as source
# group by class_id;
...
--------------------------触发器
# 创建一个日志表,来记录 有数据插入到php_student的时候就记录一下插入时间
# 注意: 字段名 尽量不要使用系统关键字,用关键字要用`包裹
...
after				-- 时间 
insert				-- 事件
on  php_student for each row -- 检测哪个表
begin				
	insert into student_log(addtime) values (now());
end
$$				-- 触发器结束
delimiter ;
-- 恢复mysql结束符

insert into php_student(name) values ('触发器');
...
insert into goods (title,kucun) values 
	('香蕉',100),		-- 1
	('苹果',100),		-- 2
	('鸭梨',100);		-- 3
...
after
insert
on goods_order for each row
begin
	update goods set kucun = kucun - 1 where id = 1;
end
||
delimiter ;


#我们在插入数据的时候,尽量在表后边写入字段
#因为指定字段之后,如果表结构发生了变化
#但是你的这个插入语句的字段还在
#那么我的sql语句不受影响
insert into goods_order (good_id,num,title) values 
	(1,1,'香蕉');
...
on goods_order for each row
begin
	update goods set kucun = kucun - new.num where id = new.good_id;
end
!!
delimiter ;
...
on goods_order for each row
begin
	update goods set kucun = kucun + old.num where id = old.good_id;
end
$$
delimiter ;

delete from goods_order where id = 2;
...
insert into trans values ('午哥','100'),('齐哥',0);

#开启事务
start transaction;


#午哥答应借给钱,这个时候银行转账,转账的时候断电了
#这个时候,午哥的钱划出来了,但是我的没到账
update trans set money = money - 100 where name = '午哥';

update trans set money = money + 100 where name = '齐哥';



create table trans_2 (
	id int primary key auto_increment,
	a varchar(10),
	b varchar(10),
	c varchar(10)
...
insert into trans_2 (a,b,c) values 
	('a','b','c'),
	('aa','bb','cc'),
	('aaa','bbb','ccc');
...
#我们让 abc这个用户可以 查询 shangke这个表
# 库名.表名 表示只能对这个表有权限
# 库名.* 表示对这个库的所有表都有这个权限
grant select on shangke.* to 'abc'@'localhost';

# 收回权限
revoke select on shangke.* from 'abc'@'localhost';

create user 'itcast'@'192.168.133.%' identified by '123456';

grant select on shangke.* to 'itcast'@'192.168.133.%'

#千万不要创建这样的用户
grant ALL PRIVILEGES on *.* to 'itcast'@'192.168.133.%';

revoke ALL PRIVILEGES on *.* from 'itcast'@'192.168.133.%';

drop user 'itcast'@'192.168.133,%';


mysql -h192.168.133.71 -P3306 -uitcast -p123456;
...
#自己创建一个函数来给名字添加书名号<< >>
delimiter $$
create function f2(name varchar(10)) returns varchar(20)
begin
	return concat('<<',name,'>>');
end
$$
...
delimiter $$
create function f3(name varchar(10)) returns varchar(20)
begin
	if name = 'itcast' then
		return '传智播客';
	elseif name = 'it' then
...
#错误的 elseif 不能分开
delimiter $$
create function f4(name varchar(10)) returns varchar(20)
begin
	if name = 'itcast' then
		return '传智播客';
	else if name = 'it' then
		return '传智';
	else 
		return '播客';
	end if;
...
#in out inout
delimiter $$
create procedure f11(in a varchar(10),out b varchar(10),inout c varchar(10))
begin
	select a,b,c;
end
$$
